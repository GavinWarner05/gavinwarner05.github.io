(function () {
  ...
  const positionGroups = [
    { id: "quarterbacks", label: "Quarterbacks", positions: ["QB"] },
    { id: "running-backs", label: "Running backs", positions: ["RB", "FB"] },
    { id: "receivers", label: "Receivers", positions: ["WR", "TE"] },
    { id: "defensive-line", label: "Defensive line", positions: ["DE", "DT", "NT", "DL"] },
    { id: "linebackers", label: "Linebackers", positions: ["LB", "ILB", "OLB"] },
    { id: "defensive-backs", label: "Defensive backs", positions: ["CB", "S", "FS", "SS", "DB"] },
    { id: "kickers", label: "Kickers", positions: ["K"] },
    { id: "punters", label: "Punters", positions: ["P"] },
    { id: "other", label: "Other", positions: [] }
  ];

  function positionGroup(position) {
    return positionGroups.find((group) => group.positions.includes(position)) || positionGroups[positionGroups.length - 1];
  }
  ...
  function totalYards(stats) {
    const values = [stats.passing_yards, stats.rushing_yards, stats.receiving_yards];
  ...
  function shareStatsForComparison(keys, firstStats, secondStats, statLabels) {
    ...
    const leftTotalYards = totalYards(firstStats);
    const rightTotalYards = totalYards(secondStats);
    if (leftTotalYards !== "—" || rightTotalYards !== "—") {
      rows.push({ key: "total_yards", label: "Total yards", leftValue: leftTotalYards, rightValue: rightTotalYards, value: `${leftTotalYards} / ${rightTotalYards}` });
    }
    return rows;
  }
  ...
  function mount(app, snapshot, player, helpers) {
    ...
    const groupSelect = el("[data-comparison-group]");
    ...
    function renderChart() {
      ...
      summary.textContent = `Select a bar to see its ${label.toLowerCase()}. Missing weeks have no reported value.`;
      ...
      const width = Math.max(620, lastWeek * 42 + 80); const height = 225;
      const left = 58; const right = width - 20; const top = 18; const bottom = 178;
      const low = Math.min(0, ...points.map((week) => week.stats[key]));
      const high = Math.max(0, ...points.map((week) => week.stats[key])) || (low < 0 ? 0 : 1);
      const valuesAreIntegers = points.every((week) => Number.isInteger(week.stats[key]));
      let axisLow = low; let axisHigh = high;
      if (valuesAreIntegers) {
        if (axisLow < 0) axisLow = -Math.ceil(Math.abs(axisLow) / 2) * 2;
        if (axisHigh > 0) axisHigh = Math.ceil(axisHigh / 2) * 2;
      }
      if (axisLow === axisHigh) axisHigh = axisLow + 1;
      const scaleY = (value) => bottom - (value - axisLow) / (axisHigh - axisLow) * (bottom - top);
      ...
      const formatter = new Intl.NumberFormat("en-US", { maximumFractionDigits: valuesAreIntegers ? 0 : 1 });
      const ticks = axisLow < 0 && axisHigh > 0 ? [axisLow, 0, axisHigh] : [axisLow, (axisLow + axisHigh) / 2, axisHigh];
      ticks.forEach((value) => {
      ...
      for (let week = 1; week <= lastWeek; week++) {
        svg.append(svgNode("text", { x: left + (week - .5) * step, y: 207, "text-anchor": "middle", class: "player-chart-label" }, `W${week}`));
      }
      points.forEach((week) => {
    ...
    function renderComparison() {
      ...
      const comparisonRows = keys.map((key) => [statLabels[key], firstStats[key] ?? "—", secondStats[key] ?? "—"]);
      const firstTotalYards = totalYards(firstStats);
      const secondTotalYards = totalYards(secondStats);
      if (firstTotalYards !== "—" || secondTotalYards !== "—") comparisonRows.push(["Total yards", firstTotalYards, secondTotalYards]);
      el("[data-comparison-table]").append(table(["Stat", player.name, otherPlayer.name], comparisonRows, periodLabel));
    ...
    function hasRecordedStats(entry) {
      const season = seasonFor(entry, otherSnapshot, currentSeason.season);
      if (!season) return false;
      if (Object.keys(season.stats || {}).length) return true;
      return (season.weekly_stats || []).some((week) => Object.keys(week.stats || {}).length);
    }

    function refreshPlayerChoices() {
      if (!otherSnapshot || !currentSeason) return;
      const previousPlayerId = otherPlayer?.id || playerSelect.value;
      const seen = new Set([player.id]);
      const eligible = (otherSnapshot.players || []).concat(otherSnapshot.injury_players || [])
        .filter((entry) => {
          if (seen.has(entry.id)) return false;
          seen.add(entry.id);
          return hasRecordedStats(entry);
        })
        .sort((a, b) => a.name.localeCompare(b.name));
      const groups = positionGroups
        .map((group) => ({ group, count: eligible.filter((entry) => positionGroup(entry.position).id === group.id).length }))
        .filter((entry) => entry.count);
      const previousGroup = groupSelect.value;
      const preferredGroup = positionGroup(player.position).id;
      groupSelect.replaceChildren(...groups.map(({ group, count }) => option(group.id, `${group.label} (${count})`)));
      groupSelect.value = groups.some(({ group }) => group.id === previousGroup)
        ? previousGroup
        : groups.some(({ group }) => group.id === preferredGroup) ? preferredGroup : groups[0]?.group.id || "";
      groupSelect.disabled = !groups.length;
      const players = eligible.filter((entry) => positionGroup(entry.position).id === groupSelect.value);
      playerSelect.replaceChildren(option("", players.length ? `Choose a player (${players.length})` : "No players available"), ...players.map((entry) => option(entry.id, `${entry.name} · ${entry.position || "NFL"}`)));
      otherPlayer = players.find((entry) => entry.id === previousPlayerId);
      if (otherPlayer) playerSelect.value = otherPlayer.id;
      playerSelect.disabled = !players.length;
      updatePeriods();
      if (!eligible.length) status.textContent = `No players on this roster have recorded ${currentSeason.season} stats yet.`;
      else if (!players.length) status.textContent = `No ${positionGroups.find((group) => group.id === groupSelect.value)?.label.toLowerCase() || "players"} have recorded ${currentSeason.season} stats.`;
      else if (!otherPlayer) status.textContent = `Choose one of ${players.length} ${players.length === 1 ? "player" : "players"} with recorded ${currentSeason.season} stats.`;
    }

    async function loadTeam() {
      const version = ++requestVersion; const id = teamSelect.value;
      otherPlayer = null; otherSnapshot = null; shareData = null;
      groupSelect.replaceChildren(option("", "Loading groups…")); groupSelect.disabled = true;
    ...
    groupSelect.addEventListener("change", () => { otherPlayer = null; refreshPlayerChoices(); });
    ...
    loadTeam();
    return { updateSeason(season) { const changed = currentSeason?.season !== season.season; currentSeason = season; if (changed) periodSelect.value = "season"; updateChart(); refreshPlayerChoices(); } };
  }
