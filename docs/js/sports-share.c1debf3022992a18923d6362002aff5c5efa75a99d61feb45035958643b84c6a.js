(function () {
  ...
  async function drawComparison(canvas, data) {
    const ctx = canvas.getContext("2d");
    background(ctx, data.teams[0].colors?.primary, data.teams[1].colors?.primary);
    brand(ctx, data.periodLabel);
    text(ctx, "PLAYER COMPARISON", 540, 132, 24, 800, "rgba(255,255,255,.76)", "center");
    const portraits = await Promise.all(data.players.map((player) => loadImage(player.headshot_url)));
    data.players.forEach((player, index) => {
      const x = index ? 790 : 290;
      fillRound(ctx, x - 200, 165, 400, 275, 28, "rgba(8,9,18,.25)");
      if (portraits[index]) contain(ctx, portraits[index], x - 165, 170, 330, 265);
      else text(ctx, player.name.split(/\s+/).map((part) => part[0]).slice(0, 2).join(""), x, 345, 94, 800, "rgba(255,255,255,.28)", "center");
      text(ctx, player.name, x, 491, fitText(ctx, player.name, 430, 40, 20, 800), 800, "#fff", "center");
      text(ctx, `${data.teams[index].abbreviation} · ${player.position || "NFL"}`, x, 530, 24, 700, "rgba(255,255,255,.76)", "center");
      text(ctx, data.contexts[index], x, 568, 21, 700, "rgba(255,255,255,.66)", "center");
    });
    text(ctx, "VS", 540, 325, 25, 800, "rgba(255,255,255,.65)", "center");
    statsForCard(data.stats).forEach((stat, index) => {
      const y = 605 + index * 94;
      fillRound(ctx, 64, y, 952, 80, 20, "rgba(8,9,18,.48)");
      text(ctx, stat.leftValue, 215, y + 53, 36, 800, "#fff", "center");
      const label = String(stat.label).toUpperCase();
      text(ctx, label, 540, y + 49, fitText(ctx, label, 390, 22, 16, 800), 800, "rgba(255,255,255,.76)", "center");
      text(ctx, stat.rightValue, 865, y + 53, 36, 800, "#fff", "center");
    });
    text(ctx, "gavinwarner.digital/sports", 64, 1030, 21, 650, "rgba(255,255,255,.55)");
    text(ctx, "— = NOT REPORTED", 1016, 1030, 18, 650, "rgba(255,255,255,.55)", "right");
  }
  ...
  function dialog() {
    ...
    modal.innerHTML = '<div class="sports-share-panel"><div class="sports-share-heading"><div><p>READY TO SEND</p><h2>Share card</h2></div><button type="button" data-share-close aria-label="Close share card">×</button></div><p class="sports-share-status" data-share-status role="status" aria-live="polite">Creating your card…</p><fieldset class="sports-share-picker" data-share-picker hidden><legend>Choose and order stats</legend><p data-share-count aria-live="polite"></p><p class="sports-share-picker-label">Card order</p><div class="sports-share-order" data-share-order></div><p class="sports-share-picker-label">Add or remove stats</p><div class="sports-share-options" data-share-options></div></fieldset><canvas width="1080" height="1080" data-share-canvas aria-label="Generated Sports Center share card"></canvas><div class="sports-share-actions"><button type="button" class="sports-share-primary" data-share-native>Share image</button><button type="button" data-share-copy>Copy image</button><button type="button" data-share-download>Save PNG</button></div></div>';
  ...
  async function renderCard(modal, kind, data) {
    const version = ++renderVersion;
    const canvas = modal.querySelector("[data-share-canvas]");
    const status = modal.querySelector("[data-share-status]");
    const buttons = modal.querySelectorAll(".sports-share-actions button");
    state.blob = null;
    buttons.forEach((button) => { button.disabled = true; });
    status.textContent = "Creating your card…";
    // Render separately so an older image load cannot overwrite a newer selection.
    const draft = document.createElement("canvas");
    draft.width = SIZE; draft.height = SIZE;
    try {
      if (kind === "player") await drawPlayer(draft, data);
      else if (kind === "comparison") await drawComparison(draft, data);
      else await drawGame(draft, data);
      const blob = await new Promise((resolve) => draft.toBlob(resolve, "image/png", .94));
      ...
      if (version === renderVersion) status.textContent = "This card could not be created. One of its images may be temporarily unavailable.";
    }
  }

  async function open(kind, data) {
    const modal = dialog();
    const picker = modal.querySelector("[data-share-picker]");
    const order = modal.querySelector("[data-share-order]");
    const options = modal.querySelector("[data-share-options]");
    const available = kind !== "game" ? (data.availableStats || data.stats || []).filter((stat) => stat && stat.value != null && stat.value !== "") : [];
    options.replaceChildren();
    picker.hidden = !available.length;
    let cardData = data;
    if (available.length) {
      const defaults = statsForCard(data.stats);
      let selected = available.filter((stat) => defaults.some((item) => item.label === stat.label)).slice(0, 4);
      if (!selected.length) selected = available.slice(0, 4);
      const inputs = available.map((stat) => {
        const label = document.createElement("label");
        const input = document.createElement("input");
        input.type = "checkbox"; input.checked = selected.includes(stat);
        const caption = document.createElement("span");
        caption.className = "sports-share-option-text";
        caption.textContent = `${stat.label}: ${stat.value}`;
        label.append(input, caption); options.append(label);
        input.addEventListener("change", function () {
          if (input.checked && selected.length < 4) selected.push(stat);
          else if (!input.checked) selected = selected.filter((item) => item !== stat);
          updateSelection();
          renderCard(modal, kind, cardData);
        });
        return { input, stat };
      });
      function moveStat(stat, direction) {
        const index = selected.indexOf(stat); const target = index + direction;
        if (index < 0 || target < 0 || target >= selected.length) return;
        [selected[index], selected[target]] = [selected[target], selected[index]];
        updateSelection(`${stat.label} moved to position ${target + 1}.`);
        renderCard(modal, kind, cardData);
      }
      function renderOrder() {
        order.replaceChildren(...selected.map((stat, index) => {
          const row = document.createElement("div"); row.className = "sports-share-order-item";
          const position = document.createElement("span"); position.className = "sports-share-order-position"; position.textContent = index + 1;
          const name = document.createElement("strong"); name.className = "sports-share-order-name"; name.textContent = stat.label;
          const value = document.createElement("small"); value.className = "sports-share-order-value"; value.textContent = stat.value;
          const actions = document.createElement("span"); actions.className = "sports-share-order-actions";
          const up = document.createElement("button"); up.type = "button"; up.textContent = "↑"; up.disabled = index === 0; up.setAttribute("aria-label", `Move ${stat.label} up`);
          const down = document.createElement("button"); down.type = "button"; down.textContent = "↓"; down.disabled = index === selected.length - 1; down.setAttribute("aria-label", `Move ${stat.label} down`);
          up.addEventListener("click", function () { moveStat(stat, -1); });
          down.addEventListener("click", function () { moveStat(stat, 1); });
          actions.append(up, down); row.append(position, name, value, actions); return row;
        }));
      }
      function updateSelection(message) {
        inputs.forEach(({ input, stat }) => {
          input.checked = selected.includes(stat);
          input.disabled = input.checked ? selected.length === 1 : selected.length >= 4;
        });
        modal.querySelector("[data-share-count]").textContent = `${message ? message + " " : ""}${selected.length} of 4 selected.`;
        cardData = Object.assign({}, data, { stats: selected.slice() });
        renderOrder();
      }
      updateSelection();
    }
    ...
    return renderCard(modal, kind, cardData);
  }

  window.SportsShare = {
    player: function (data) { return open("player", data); },
    comparison: function (data) { return open("comparison", data); },
