(function () {
  ...
  const els = {
    ...
    playerSearchSection: app.querySelector("[data-player-search-section]"),
    playerSearch: app.querySelector("[data-player-search]"),
    playerSearchStatus: app.querySelector("[data-player-search-status]"),
    playerSearchResults: app.querySelector("[data-player-search-results]"),
    playerSearchCount: app.querySelector("[data-player-search-count]"),
    leadersSection: app.querySelector("[data-weekly-leaders-section]"),
    leadersHeading: app.querySelector("[data-leaders-heading]"),
    leadersWeek: app.querySelector("[data-leaders-week]"),
    leaderCategories: app.querySelector("[data-leader-categories]"),
    leaders: app.querySelector("[data-weekly-leaders]"),
  ...
  const state = { data: null, teams: null, playerIndex: null, playerTeams: null, playersById: null, leaderCategory: null };
  ...
  function playerHref(player) {
    return app.dataset.playerUrl + "?team=" + encodeURIComponent(player.team_id) + "&id=" + encodeURIComponent(player.id);
  }
  ...
  function playerPortrait(player, className) {
    ...
    if (player.headshot_url) {
      const image = document.createElement("img");
      image.src = optimizedHeadshot(player.headshot_url, 180);
      image.alt = "";
      image.loading = "lazy";
      image.decoding = "async";
      image.addEventListener("error", function () { image.remove(); portrait.textContent = player.position || "NFL"; }, { once: true });
      portrait.append(image);
    } else portrait.textContent = player.position || "NFL";
    return portrait;
  }

  function searchScore(player, team, query) {
    const name = normalized(player.name);
    const position = normalized(player.position);
    const teamName = normalized(team && team.name);
    const abbreviation = normalized(team && team.abbreviation);
    if (name === query) return 0;
    if (name.startsWith(query)) return 1;
    if (name.split(/\s+/).some((word) => word.startsWith(query))) return 2;
    if (name.includes(query)) return 3;
    if (position === query) return 4;
    if (teamName.startsWith(query) || abbreviation === query) return 5;
    if (teamName.includes(query) || position.includes(query)) return 6;
    return -1;
  }
  ...
  function renderPlayerSearch() {
    const query = normalized(els.playerSearch.value);
    if (query.length < 2) {
      els.playerSearchStatus.textContent = query ? "Type one more character to search." : "Start typing a player, team, or position.";
      els.playerSearchResults.replaceChildren();
      closePlayerSearch();
      return;
    }
    const matches = state.playerIndex.players.map((player) => {
      const team = state.playerTeams.get(player.team_id);
      return { player: player, team: team, score: searchScore(player, team, query) };
    }).filter((entry) => entry.score >= 0).sort((a, b) => a.score - b.score || a.player.name.localeCompare(b.player.name) || a.player.team_id.localeCompare(b.player.team_id)).slice(0, 8);

    const links = matches.map((entry) => {
      const link = document.createElement("a");
      link.className = "sports-player-search-result";
      link.href = playerHref(entry.player);
      link.append(playerPortrait(entry.player, "sports-player-search-photo"));
      const copy = document.createElement("span");
      copy.className = "sports-player-search-copy";
      copy.append(node("strong", entry.player.name), node("small", (entry.team ? entry.team.abbreviation : entry.player.team_id.toUpperCase()) + " · " + (entry.player.position || "Player")));
      link.append(copy, node("span", "View →", "sports-player-search-action"));
      return link;
    });
    els.playerSearchResults.replaceChildren(...links);
    els.playerSearchResults.hidden = false;
    els.playerSearch.setAttribute("aria-expanded", "true");
    els.playerSearchStatus.textContent = matches.length ? matches.length + " result" + (matches.length === 1 ? "" : "s") + " shown." : "No players found. Try a full name, team, or position.";
    if (!matches.length) els.playerSearchResults.append(node("p", "No matching players", "sports-player-search-empty"));
  }

  function renderLeaderCategory() {
    const selectedWeek = Number(els.leadersWeek.value);
    const week = state.playerIndex.weeks.find((entry) => entry.week === selectedWeek);
    if (!week || !week.categories.length) { els.leadersSection.hidden = true; return; }
    const keys = week.categories.map((category) => category.key);
    if (!keys.includes(state.leaderCategory)) state.leaderCategory = keys[0];
    els.leaderCategories.replaceChildren(...week.categories.map((category) => {
      const button = node("button", category.label);
      button.type = "button";
      button.dataset.leaderCategory = category.key;
      button.setAttribute("aria-pressed", String(category.key === state.leaderCategory));
      button.addEventListener("click", function () { state.leaderCategory = category.key; renderLeaderCategory(); });
      return button;
    }));
    const category = week.categories.find((entry) => entry.key === state.leaderCategory);
    els.leadersHeading.textContent = state.playerIndex.season + " · Week " + week.week;
    const rows = category.leaders.map((leader, index) => {
      const player = state.playersById.get(leader.team_id + ":" + leader.id) || leader;
      const team = state.playerTeams.get(leader.team_id);
      const link = document.createElement("a");
      link.className = "sports-leader-row" + (index === 0 ? " is-first" : "");
      link.href = playerHref(Object.assign({}, player, { team_id: leader.team_id }));
      link.append(node("span", index + 1, "sports-leader-rank"), playerPortrait(player, "sports-leader-photo"));
      const copy = document.createElement("span");
      copy.className = "sports-leader-copy";
      copy.append(node("strong", leader.name), node("small", (team ? team.abbreviation : leader.team_id.toUpperCase()) + " · " + (leader.position || "Player")));
      const value = document.createElement("span");
      value.className = "sports-leader-value";
      value.append(node("strong", Number(leader.value).toLocaleString("en-US")), node("small", category.suffix));
      link.append(copy, value);
      return link;
    });
    els.leaders.replaceChildren(...rows);
    els.leadersSection.hidden = rows.length === 0;
  }

  function renderWeeklyLeaders() {
    const weeks = state.playerIndex.weeks.slice().sort((a, b) => b.week - a.week);
    if (!weeks.length) return;
    els.leadersWeek.replaceChildren(...weeks.map((week) => {
      const option = node("option", "Week " + week.week);
      option.value = week.week;
      return option;
    }));
    els.leadersWeek.value = weeks[0].week;
    renderLeaderCategory();
  }

  function loadPlayerDiscovery() {
    return fetch(app.dataset.playerIndexUrl, { headers: { Accept: "application/json" }, cache: "no-cache" })
      .then((response) => { if (!response.ok) throw new Error("HTTP " + response.status); return response.json(); })
      .then((index) => {
        if (!Array.isArray(index.players) || !Array.isArray(index.teams) || !Array.isArray(index.weeks)) throw new Error("Invalid player index");
        state.playerIndex = index;
        state.playerTeams = new Map(index.teams.map((team) => [team.id, team]));
        state.playersById = new Map(index.players.map((player) => [player.team_id + ":" + player.id, player]));
        els.playerSearchCount.textContent = index.players.length.toLocaleString("en-US") + " players";
        els.playerSearchSection.hidden = false;
        renderWeeklyLeaders();
      })
      .catch(() => { els.playerSearchSection.hidden = true; els.leadersSection.hidden = true; });
  }
  ...
  els.playerSearch.addEventListener("input", function () { if (state.playerIndex) renderPlayerSearch(); });
  els.playerSearch.addEventListener("focus", function () { if (state.playerIndex && normalized(els.playerSearch.value).length >= 2) renderPlayerSearch(); });
  els.playerSearch.addEventListener("keydown", function (event) {
    if (event.key === "Escape") { els.playerSearch.value = ""; renderPlayerSearch(); }
  });
  els.leadersWeek.addEventListener("change", renderLeaderCategory);
  document.addEventListener("click", function (event) {
    if (!event.target.closest(".sports-player-search-shell")) closePlayerSearch();
  });
