(function () {
  ...
  function render(data, player) {
    ...
    if (injury) {
      ...
      const status = String(injury.status || "Listed");
      injuryBadge.textContent = status + (injury.detail ? " · " + injury.detail : "");
      injuryBadge.classList.add(/questionable/i.test(status) ? "is-questionable" : /doubtful/i.test(status) ? "is-doubtful" : /out|ir|pup|suspend/i.test(status) ? "is-out" : "is-other");
      injuryBadge.hidden = false;
    }
    updateFavorite(player);
    el("[data-player-favorite]").addEventListener("click", function () { saveFavorite(player, team); });
    app.querySelectorAll("[data-player-content]").forEach((section) => { section.hidden = false; });
  ...
  fetch(app.dataset.teamDataBaseUrl + encodeURIComponent(teamId) + ".json", { headers: { Accept: "application/json" }, cache: "no-cache" })
    ...
    .then((data) => {
      const player = (data.players || []).concat(data.injury_players || []).find((entry) => entry.id === playerId);
