(function () {
    ...
    data.games.slice().sort((left, right) => new Date(left.kickoff) - new Date(right.kickoff)).filter((game) => {
      ...
      [[away, home, game.away_score, game.home_score], [home, away, game.home_score, game.away_score]].forEach(([record, opponent, ownScore, otherScore]) => {
        ...
        record.games.push({
          ...
          result: resultLetter(ownScore, otherScore),
          conferenceGame: record.conference === opponent.conference,
          divisionGame: record.conference === opponent.conference && record.division === opponent.division
        });
  ...
  function percentageFromGames(games) {
  ...
  function headToHeadPercentage(record, candidates) {
    const ids = new Set(candidates.map((candidate) => candidate.team.id));
    return percentageFromGames(record.games.filter((game) => ids.has(game.opponentId)));
  }

  function commonOpponentIds(candidates) {
    if (!candidates.length) return new Set();
    const common = new Set(candidates[0].games.map((game) => game.opponentId));
    candidates.slice(1).forEach((record) => {
      const opponents = new Set(record.games.map((game) => game.opponentId));
      Array.from(common).forEach((id) => { if (!opponents.has(id)) common.delete(id); });
    });
    return common;
  }

  function commonGamesPercentage(record, opponentIds, minimumGames) {
    const games = record.games.filter((game) => opponentIds.has(game.opponentId));
    return games.length >= minimumGames ? percentageFromGames(games) : null;
  }

  function opponentStrength(record, records, victoriesOnly) {
    const games = victoriesOnly ? record.games.filter((game) => game.result === "W") : record.games;
    let wins = 0;
    let losses = 0;
    let ties = 0;
    games.forEach((game) => {
      const opponent = records.get(game.opponentId);
      if (!opponent) return;
      wins += opponent.wins;
    ...
    return total ? (wins + ties * 0.5) / total : 0;
  }

  function competitionRank(record, pool, field, descending) {
    const value = record[field];
    return 1 + pool.filter((other) => descending ? other[field] > value : other[field] < value).length;
  }

  function combinedPointsRank(record, pool) {
    return competitionRank(record, pool, "pointsFor", true) + competitionRank(record, pool, "pointsAgainst", false);
  }

  function netPoints(record, predicate) {
    return record.games.filter(predicate || (() => true)).reduce((total, game) => total + game.ownScore - game.otherScore, 0);
  }

  function applyCriteria(candidates, criteria, restart) {
    for (const criterion of criteria) {
      if (!criterion) continue;
      const values = candidates.map((record) => criterion(record));
      if (values.some((value) => value == null || !Number.isFinite(value))) continue;
      const best = Math.max(...values);
      const leaders = candidates.filter((record, index) => Math.abs(values[index] - best) < 1e-9);
      if (leaders.length === 1) return leaders[0];
      if (leaders.length < candidates.length) return restart(leaders);
    }
    return candidates.slice().sort((left, right) => left.team.name.localeCompare(right.team.name))[0];
  }

  function selectDivisionWinner(candidates, records, conferencePool, leaguePool) {
    if (candidates.length === 1) return candidates[0];
    const common = commonOpponentIds(candidates);
    const headToHeadApplicable = candidates.every((record) => headToHeadPercentage(record, candidates) != null);
    const commonApplicable = common.size > 0 && candidates.every((record) => commonGamesPercentage(record, common, 1) != null);
    const criteria = [
      (record) => winPercentage(record),
      (record) => record.wins,
      headToHeadApplicable ? (record) => headToHeadPercentage(record, candidates) : null,
      (record) => winPercentage(record.divisionRecord),
      commonApplicable ? (record) => commonGamesPercentage(record, common, 1) : null,
      (record) => winPercentage(record.conferenceRecord),
      (record) => opponentStrength(record, records, true),
      (record) => opponentStrength(record, records, false),
      (record) => -combinedPointsRank(record, conferencePool),
      (record) => -combinedPointsRank(record, leaguePool),
      commonApplicable ? (record) => netPoints(record, (game) => common.has(game.opponentId)) : null,
      (record) => netPoints(record)
    ];
    return applyCriteria(candidates, criteria, (leaders) => selectDivisionWinner(leaders, records, conferencePool, leaguePool));
  }

  function rankDivision(candidates, records, conferencePool, leaguePool) {
    const remaining = candidates.slice();
    const ranked = [];
    while (remaining.length) {
      const winner = selectDivisionWinner(remaining, records, conferencePool, leaguePool);
      ranked.push(winner);
  ...
  function wildcardSweep(candidates) {
    if (candidates.length < 3) return null;
    const opponentIds = new Set(candidates.map((record) => record.team.id));
    const summaries = candidates.map((record) => {
      const games = record.games.filter((game) => opponentIds.has(game.opponentId));
      const opponentsPlayed = new Set(games.map((game) => game.opponentId));
      return {
        record: record,
        covered: opponentsPlayed.size === candidates.length - 1,
        wonAll: games.length > 0 && games.every((game) => game.result === "W"),
        lostAll: games.length > 0 && games.every((game) => game.result === "L")
      };
    });
    const winner = summaries.find((summary) => summary.covered && summary.wonAll);
    if (winner) return candidates.filter((record) => record === winner.record);
    const loser = summaries.find((summary) => summary.covered && summary.lostAll);
    if (loser) return candidates.filter((record) => record !== loser.record);
    return null;
  }

  function divisionRepresentatives(candidates, divisionRanks) {
    const represented = new Set();
    const representatives = [];
    candidates.forEach((record) => {
      if (represented.has(record.division)) return;
      represented.add(record.division);
      const order = divisionRanks.get(record.division) || [];
      representatives.push(order.find((candidate) => candidates.includes(candidate)) || record);
    });
    return representatives;
  }

  function selectWildcardWinner(candidates, records, conferencePool, leaguePool, divisionRanks) {
    if (candidates.length === 1) return candidates[0];
    const representatives = divisionRepresentatives(candidates, divisionRanks);
    if (representatives.length === 1) {
      return selectDivisionWinner(candidates, records, conferencePool, leaguePool);
    }
    candidates = representatives;
    if (candidates.length === 1) return candidates[0];
    if (candidates.length > 2) {
      const sweep = wildcardSweep(candidates);
      if (sweep && sweep.length === 1) return sweep[0];
      if (sweep && sweep.length < candidates.length) {
        return selectWildcardWinner(sweep, records, conferencePool, leaguePool, divisionRanks);
      }
    }
    const common = commonOpponentIds(candidates);
    const commonApplicable = common.size > 0 && candidates.every((record) => commonGamesPercentage(record, common, 4) != null);
    const headToHeadApplicable = candidates.length === 2 && candidates.every((record) => headToHeadPercentage(record, candidates) != null);
    const criteria = [
      (record) => winPercentage(record),
      (record) => record.wins,
      headToHeadApplicable ? (record) => headToHeadPercentage(record, candidates) : null,
      (record) => winPercentage(record.conferenceRecord),
      commonApplicable ? (record) => commonGamesPercentage(record, common, 4) : null,
      (record) => opponentStrength(record, records, true),
      (record) => opponentStrength(record, records, false),
      (record) => -combinedPointsRank(record, conferencePool),
      (record) => -combinedPointsRank(record, leaguePool),
      (record) => netPoints(record, (game) => game.conferenceGame),
      (record) => netPoints(record)
    ];
    return applyCriteria(candidates, criteria, (leaders) => selectWildcardWinner(leaders, records, conferencePool, leaguePool, divisionRanks));
  }

  function rankWildcardCandidates(candidates, records, conferencePool, leaguePool, divisionRanks) {
    const remaining = candidates.slice();
    const ranked = [];
    while (remaining.length) {
      const winner = selectWildcardWinner(remaining, records, conferencePool, leaguePool, divisionRanks);
      ranked.push(winner);
      remaining.splice(remaining.indexOf(winner), 1);
    }
    return ranked;
  ...
  function rankLeagueRecords(records) {
    const pool = Array.from(records.values());
    return pool.slice().sort((left, right) => {
      return winPercentage(right) - winPercentage(left)
        || right.wins - left.wins
        || opponentStrength(right, records, true) - opponentStrength(left, records, true)
        || opponentStrength(right, records, false) - opponentStrength(left, records, false)
        || combinedPointsRank(left, pool) - combinedPointsRank(right, pool)
        || netPoints(right) - netPoints(left)
  ...
  function conferenceEntries(conference, records) {
    ...
    const leaguePool = Array.from(records.values());
    const divisionRanks = new Map(Object.entries(structure[conference]).map(([division, ids]) => {
      const candidates = ids.map((id) => records.get(id)).filter(Boolean);
      return [division, rankDivision(candidates, records, conferenceRecords, leaguePool)];
    }));
    const divisionLeaders = rankWildcardCandidates(Array.from(divisionRanks.values()).map((ranked) => ranked[0]), records, conferenceRecords, leaguePool, divisionRanks);
    const leaderIds = new Set(divisionLeaders.map((record) => record.team.id));
    const remaining = rankWildcardCandidates(conferenceRecords.filter((record) => !leaderIds.has(record.team.id)), records, conferenceRecords, leaguePool, divisionRanks);
  ...
  function renderDetailed(view) {
    ...
    const entries = isLeague
      ? rankLeagueRecords(state.records).map((record, index) => ({ record: record, rank: index + 1, status: "outside" }))
